...
class MyGroupNorm(nn.GroupNorm):
    def __init__(self, num_channels, eps=1e-5, affine=True, num_groups=16):
...
class Net(nn.Module):
	...
	def forward(self, batch):
		...
		if 'loss' in self.output_type:
			output['loss'] = focal_loss(logit, batch['mask']) * self.args.Focal_loss_weight \
							+ criterion_dice_loss(logit, batch['mask']) * self.args.Dice_loss_weight
